...
class EMAEPretrainDecoder(BaseModule):
    """Decoder for EMAE Pre-training.
    ...
    def __init__(self,
        ...
        super(EMAEPretrainDecoder, self).__init__()
    ...
    def init_weights(self):
        super(EMAEPretrainDecoder, self).init_weights()
    ...
    def forward(self, x, ids_restore):
        # embed tokens
        outcome = self.decoder_embed(x)

        # append mask tokens to sequence
        mask_tokens = self.mask_token.repeat(
            outcome.shape[0], ids_restore.shape[1] + 1 - outcome.shape[1], 1)
        x_ = torch.cat([outcome[:, 1:, :], mask_tokens], dim=1)
        x_ = torch.gather(
            x_,
            dim=1,
            index=ids_restore.unsqueeze(-1).repeat(1, 1, outcome.shape[2]))
        outcome = torch.cat([outcome[:, :1, :], x_], dim=1)

        # add pos embed
        outcome = outcome + self.decoder_pos_embed

        # apply Transformer blocks
        for blk in self.decoder_blocks:
            outcome = blk(outcome)
        outcome = self.decoder_norm(outcome)

        # predictor projection
        outcome = self.decoder_pred(outcome)
